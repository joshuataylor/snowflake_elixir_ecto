defmodule Ecto.Adapters.Snowflake.Connection do
  ...
  def child_spec(opts) do
    SnowflakeEx.child_spec(opts)
  end

  @impl true
  def query(conn, sql, params, opts) do
    SnowflakeEx.query(conn, sql, params, opts)
  end

  @impl true
  def prepare_execute(conn, name, sql, params, opts) do
    SnowflakeEx.prepare_execute(conn, name, sql, params, opts)
  end
  ...
  def execute(conn, sql, params, opts) do
    SnowflakeEx.execute(conn, sql, params, opts)
  end
  ...
  def explain_query(conn, query, params, opts) do
    {explain_opts, opts} = Keyword.split(opts, ~w[explain]a)

    case query(conn, build_explain_query(query, explain_opts), params, opts) do
      {:ok, %SnowflakeEx.Result{rows: rows}} -> {:ok, Enum.map_join(rows, "\n", & &1)}
      error -> error
    end
  end

  def build_explain_query(query, _opts) do
    [
      "EXPLAIN ",
      query
    ]
  ...
  defp from(%{from: %{hints: [_ | _]}} = query, _sources) do
    error!(query, "table hints are not supported by Snowflake")
  end
  ...
  defp using_join(%{joins: joins} = query, kind, prefix, sources) do
    froms =
      intersperse_map(joins, ", ", fn
        ...
        %JoinExpr{qual: qual} ->
          error!(query, "Snowflake supports only inner joins on #{kind}, got: `#{qual}`")
      end)
  ...
  defp expr({:in, _, [left, {:^, _, [_ix, _]}]}, sources, query) do
  ...
  defp expr(%Ecto.Query.Tagged{value: binary, type: :binary}, _sources, _query)
    ...
    ["'\\x", Base.encode16(binary, case: :lower) | "'::binary"]
  end
  ...
  defp tagged_to_db({:array, _}), do: "array"
  ...
  def execute_ddl({command, %Index{unique: true} = index}) when command in @creates do
  ...
  def execute_ddl(keyword) when is_list(keyword),
    do: error!(nil, "Snowflake adapter does not support keyword lists in execute")
  ...
  def pk_definition(columns, prefix) do
    pks =
  ...
  def comments_on(object, name, comment) do
    [["COMMENT ON ", object, ?\s, name, " IS ", single_quote(comment)]]
  end

  def comments_on(_object, _name, nil, _table_name), do: []

  def comments_on(object, name, comment, table_name) do
    [
      [
        "COMMENT ON ",
        object,
  ...
  def comments_for_columns(table_name, columns) do
  ...
  def column_definitions(table, columns) do
  ...
  def options_expr(nil),
    do: []

  def options_expr(keyword) when is_list(keyword),
    do: error!(nil, "Snowflake adapter does not support keyword lists in :options")

  def options_expr(options),
    do: [?\s, options]
  ...
  defp ecto_to_db({:array, _t}), do: "array"
  defp ecto_to_db(:id), do: "integer"
  defp ecto_to_db(:identity), do: "bigint"
  defp ecto_to_db(:serial), do: "serial"
  defp ecto_to_db(:bigserial), do: "bigint"
  defp ecto_to_db(:binary_id), do: "varchar"
  defp ecto_to_db(:string), do: "varchar"
  defp ecto_to_db(:binary), do: "binary"
  defp ecto_to_db(:map), do: "variant"
  defp ecto_to_db({:map, _}), do: "variant"
  defp ecto_to_db(:time_usec), do: "time"
  defp ecto_to_db(:uuid), do: "text"
  defp ecto_to_db(:array), do: "array"
  defp ecto_to_db(:bit), do: "text"
