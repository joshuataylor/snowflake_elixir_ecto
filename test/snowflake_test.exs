defmodule Ecto.Adapters.SnowflakeTest do
  use ExUnit.Case, async: false

  import Ecto.Query

  alias Ecto.Queryable
  alias Ecto.Adapters.Snowflake.Connection, as: SQL
  ...
  defmodule Schema do
    ...
    schema "schema" do
      ...
      has_many(:comments, Ecto.Adapters.SnowflakeTest.Schema2,
        references: :x,
        foreign_key: :z
      )

      has_one(:permalink, Ecto.Adapters.SnowflakeTest.Schema3,
        references: :y,
        foreign_key: :id
      )
    end
  ...
  defmodule Schema2 do
    ...
    schema "schema2" do
      belongs_to(:post, Ecto.Adapters.SnowflakeTest.Schema,
        references: :x,
        foreign_key: :z
      )
    end
  ...
  defp plan(query, operation \\ :all) do
    {query, _params} =
      Ecto.Adapter.Queryable.plan_query(operation, Ecto.Adapters.Snowflake, query)
  ...
  test "from with hints" do
    assert_raise Ecto.QueryError, ~r/table hints are not supported by Snowflake/, fn ->
  ...
  test "from without schema" do
    ...
    assert_raise Ecto.QueryError, ~r"Snowflake does not support selecting all fields from \"posts\" without a schema", fn ->
  ...
  test "literals" do
    ...
    assert all(query) == "SELECT TRUE FROM \"schema\" AS s0 WHERE (s0.\"foo\" = 'true')"

    query = "schema" |> where(foo: false) |> select([], true) |> plan()
    assert all(query) == "SELECT TRUE FROM \"schema\" AS s0 WHERE (s0.\"foo\" = 'false')"
    ...
    assert all(query) == ~s{SELECT TRUE FROM "schema" AS s0 WHERE (s0."foo" = '\\x00616263'::binary)}

    query = "schema" |> where(foo: 123) |> select([], true) |> plan()
    assert all(query) == "SELECT TRUE FROM \"schema\" AS s0 WHERE (s0.\"foo\" = '123')"

    query = "schema" |> where(foo: 123.0) |> select([], true) |> plan()
    assert all(query) == "SELECT TRUE FROM \"schema\" AS s0 WHERE (s0.\"foo\" = '123.0')"
  end
  ...
  test "tagged type" do
    ...
    assert all(query) == ~s{SELECT ?::text FROM "schema" AS s0}

    query = Schema |> select([], type(^["601d74e4-a8d3-4b6e-8365-eddb4c893327"], {:array, Ecto.UUID})) |> plan()
    assert all(query) == ~s{SELECT ?::array FROM "schema" AS s0}
  end
  ...
  test "fragments and types" do
    ...
    result =
      ...
      "WHERE (extract(? from s0.\"start_time\") = ?::bigint) " <>
      "AND (extract(? from s0.\"start_time\") = ?::bigint)"
  ...
  test "build_explain_query" do
    ...
    assert SQL.build_explain_query("SELECT 1", analyze: nil) == "EXPLAIN SELECT 1"
  end
  ...
  describe "query interpolation parameters" do
    ...
    test "join on subquery with simple select" do
      ...
      assert all(query) ==
               ...
               ~s{(SELECT ? AS "x", ? AS "w" FROM "schema" AS ss0) AS s1 ON TRUE } <>
               ~s{WHERE (s0."x" = ?)}
    end
  end
...
#    assert query == ~s{INSERT INTO "schema" ("x","y") VALUES (?,?)}
  ...
  test "insert with query" do
    ...
    assert query == ~s{INSERT INTO "schema" ("x","y","z") VALUES (?,(SELECT s0."id" FROM "schema" AS s0),?),(DEFAULT,(SELECT s0."id" FROM "schema" AS s0),?)}
  end
  ...
  test "create table" do
    ...
    assert execute_ddl(create) == ["""
                                   ...
                                   "tags" array DEFAULT ARRAY[]::array,
                                   "languages" array DEFAULT ARRAY['pt','es']::array,
                                   "limits" array DEFAULT ARRAY[100,30000]::array)
                                   """ |> remove_newlines]
  end
  ...
  test "create table with binary column and UTF-8 default" do
    ...
    assert execute_ddl(create) == ["""
                                   CREATE TABLE "blobs" ("blob" binary DEFAULT 'foo')
                                   """ |> remove_newlines]
  end

  test "create table with binary column and hex binary literal default" do
    create = {:create, table(:blobs),
      [{:add, :blob, :binary, [default: "\\x666F6F"]}]}

    assert execute_ddl(create) == ["""
                                   CREATE TABLE "blobs" ("blob" binary DEFAULT '\\x666F6F')
                                   """ |> remove_newlines]
  end

  test "create table with binary column and hex binary literal null-byte" do
    create = {:create, table(:blobs),
      [{:add, :blob, :binary, [default: "\\x00"]}]}

    assert execute_ddl(create) == ["""
                                   CREATE TABLE "blobs" ("blob" binary DEFAULT '\\x00')
                                   """ |> remove_newlines]
  end
  ...
  test "create table with a map column, and an empty map default" do
    ...
    assert execute_ddl(create) == [~s|CREATE TABLE "posts" ("a" variant DEFAULT '{}')|]
  end
  ...
  test "create table with a map column, and a map default with values" do
    ...
    assert execute_ddl(create) == [~s|CREATE TABLE "posts" ("a" variant DEFAULT '{"baz":"boom","foo":"bar"}')|]
  end
  ...
  test "create table with a map column, and a string default" do
    ...
    assert execute_ddl(create) == [~s|CREATE TABLE "posts" ("a" variant DEFAULT '{"foo":"bar","baz":"boom"}')|]
  end
  ...
  test "create table with time columns" do
    ...
    assert execute_ddl(create) == [~s|CREATE TABLE "posts" ("published_at" time, "submitted_at" time)|]
  end
  ...
  test "create table with utc_datetime columns" do
    ...
    assert execute_ddl(create) == [~s|CREATE TABLE "posts" ("published_at" timestamp, "submitted_at" timestamp)|]
  end
  ...
  test "alter table" do
    ...
    assert execute_ddl(alter) == ["""
                                  ...
                                  ADD COLUMN "category_id" bigint CONSTRAINT "posts_category_id_fkey" REFERENCES "categories"("id"),
  ...
  test "create index" do
    ...
    assert_raise ArgumentError, "Snowflake adapter does not support this index type", fn ->
      execute_ddl(create)
    end
  ...
  test "create unique index" do
    ...
    assert execute_ddl(create) ==
             [~s|ALTER TABLE "posts" ADD UNIQUE ("permalink")|]
  end
  ...
  test "create invalid constraint" do
    ...
    assert execute_ddl(create) == [~s|ALTER TABLE "foo"."products" ADD CONSTRAINT "price_must_be_positive" CHECK (price > '0')|]
  end
